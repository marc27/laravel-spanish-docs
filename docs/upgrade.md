...
Actualice su dependencia de `laravel/framework` a `^7.0` en su archivo `composer.json`. Además, actualice su dependencia `nunomaduro/collision` a `^4.1`, la dependencia de`phpunit/phpunit` a `^8.5` y la de `facade/ignition` a `^2.0`.
